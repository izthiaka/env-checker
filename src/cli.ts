...
import { EnvChecker, validators, transformers, createValidationConfig } from './index';
...
program
  ...
  .option('-r, --required <vars>', 'Variables requises (séparées par des virgules)')
  .option('-o, --optional <vars>', 'Variables optionnelles (séparées par des virgules)')
  ...
  .action((options) => {
    ...
    if (!result.isValid) {
      ...
      if (result.validationErrors.length > 0) {
        console.error('Erreurs de validation:', result.validationErrors.join('\n'));
      }
      process.exit(1);
    } else {
...
program
  ...
  .option('-f, --format <format>', 'Format de validation (email, url, uuid, date, json, port, ip, semver, hex, base64)')
  .option('-r, --regex <pattern>', 'Pattern regex personnalisé')
  .option('-t, --transform <transform>', 'Transformation à appliquer (lowerCase, upperCase, trim, toNumber, toBoolean, toArray, toObject)')
  .action((varName, value, options) => {
    const validationConfig = createValidationConfig({
      ...
      regex: options.regex ? { [varName]: new RegExp(options.regex) } : undefined,
      transform: options.transform ? { [varName]: (transformers as any)[options.transform] } : undefined,
    });
...
program
  ...
  .option('-t, --type <type>', 'Type d\'exemple (nestjs, nextjs, express)', 'nestjs')
  .action((options) => {
    const examples = {
      nestjs: `
// Exemple pour NestJS
